...
def overall_diagnosis(results: list[ProbeResult]) -> dict[str, Any]:
    ...
    top_overhead = sorted(results, key=lambda r: r.sc_median_ns, reverse=True)[:5]
    top_overhead_cases = [
        (r.operation_name, r.size, r.backend, r.sc_median_ns) for r in top_overhead
    ]
    ...
    for r in results:
        family_buckets.setdefault(r.family, []).append(r.speedup)
    family_overhead_ranking = sorted(
        ((family, float(median(speeds))) for family, speeds in family_buckets.items()),
