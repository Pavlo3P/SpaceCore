...
def _error_vs_reference(actual: Any, reference: Any) -> float:
    ...
    if hasattr(actual, "value") and hasattr(actual, "iterations"):
        # CG / LSQR / power_iteration result objects.
        actual = actual.value
    try:
...
def run_probes(
    ...
    *,
    ...
    plan: list[tuple[Probe, str, str, int]] = []
    for probe in probes:
        ...
        for backend in eligible_backends:
            ...
            for device in iter_devs:
                for size in probe.sizes:
                    plan.append((probe, backend, device, size))
    total = len(plan)
    for i, (probe, backend, device, size) in enumerate(plan, 1):
        if progress:
            tag = f"{backend}/{device}" if probe.device_aware else backend
            print(f"[{i}/{total}] {probe.name} {tag} n={size}", flush=True)
        seed_records: list[SeedTiming] = []
        ...
        for seed in seeds:
            try:
                ...
                continue
        if seed_records:
            results.append(_aggregate(probe, backend, device, size, seed_records))
    return results
