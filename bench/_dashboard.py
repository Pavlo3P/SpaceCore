...
5. **Interactive Plotly charts** (rendered client-side from
   ``window.BENCH_DATA``):
   - Speedup distribution (log-x histogram per backend with verdict-band guides).
   - Per-(family, backend) box plot (log-y).
   - Scaling curves (log-log, one line per (operation, backend)).
   - Per-seed jitter scatter (log-x, colored by backend).
   - Per-family median peak Python bytes (grouped bars by backend).
...
6. **Sortable, filterable table** with one row per
   ``(probe, size, backend)`` case. Vanilla JS; no framework
...
def _key(r: ProbeResult) -> str:
    # Backend is part of the key so multi-backend rows never collide.
    return f"{r.operation_name}@{r.size}@{r.backend}"
...
def _diagnosis_to_dict(verdict) -> dict:
    ...
    * a mapping with at least ``reason`` (and optionally ``summary``).
    ...
    if isinstance(verdict, dict):
        ...
        reason = getattr(verdict, "reason", "NEUTRAL")
...
def _build_overall(
    ...
    """
    # ----- panel 3: top overhead cases (slowest SC times) --------------
    by_overhead = sorted(results, key=lambda r: r.sc_median_ns, reverse=True)
    top_overhead = [
        {
...
<div class="diagnosis-grid" id="diagnosis-section">
  ...
  <div class="diag-panel" id="diag-overhead">
    <div class="diag-title">Top overhead cases</div>
...
_PLOT_BASELINE_BLOCK = '<div id="plot-baseline" class="plot wide"></div>'
...
(function () {
  ...
  function speedupDistribution(rows) {
    const present = backendsPresent(rows);
    ...
    const traces = present.map(function (b) {
      const xs = rows
        .filter(function (r) { return r.backend === b; })
      ...
      return {
        ...
        marker: { color: backendColor(b), line: { width: 1, color: "#1F3552" } },
        opacity: 0.7,
        hovertemplate: b + "<br>speedup: %{x:.3f} (log10)<br>cases: %{y}<extra></extra>",
      };
    });
    const shapes = [];
    [
  ...
  function familyBoxplot(rows) {
    const present = backendsPresent(rows);
    const traces = present.map(function (b) {
      const xs = [], ys = [];
      FAMILIES.forEach(function (f) {
        rows.filter(function (r) { return r.family === f && r.backend === b; })
            .forEach(function (r) { xs.push(f); ys.push(r.speedup); });
      });
      return {
        x: xs, y: ys,
        name: b, type: "box", boxmean: true,
        offsetgroup: b,
        marker: { color: backendColor(b) },
        line: { color: backendColor(b) },
      };
    });
    const layout = Object.assign({}, LAYOUT_BASE, {
      title: "2. Per-family speedup spread, grouped by backend (log-y)",
      yaxis: { type: "log", title: "speedup" },
      xaxis: { title: "family" },
      boxmode: "group",
      showlegend: true,
      legend: { orientation: "h", y: -0.2 },
    ...
    Plotly.react("plot-boxplot", traces, layout, { displaylogo: false, responsive: true });
  }

  function scalingCurves(rows) {
    // One line per (operation, backend).
    const byKey = {};
    rows.forEach(function (r) {
      const k = r.operation_name + "||" + r.backend;
    ...
    Object.keys(byKey).sort().forEach(function (k) {
      ...
      traces.push({
        ...
        y: pts.map(function (p) { return p.sc_median_ns; }),
        name: opname + " [" + backend + "]",
        mode: "lines+markers",
        type: "scatter",
        line: { color: backendColor(backend), width: 1.5 },
    ...
    const layout = Object.assign({}, LAYOUT_BASE, {
      title: "3. Scaling curves (log-log; one line per operation+backend)",
      xaxis: { type: "log", title: "problem size" },
      yaxis: { type: "log", title: "SC median time (ns)" },
      showlegend: false,
    });
  ...
  function perSeedJitter(rows) {
    ...
    const traces = present.map(function (b) {
      const xs = [], ys = [], texts = [];
      rows.filter(function (r) { return r.backend === b; }).forEach(function (r) {
        if (r.sc_median_ns <= 0) return;
        (r.seeds || []).forEach(function (s) {
          xs.push(r.size);
          ys.push(s.sc_median_ns / r.sc_median_ns);
          texts.push(r.operation_name + " [" + r.backend + "]<br>seed " + s.seed);
        });
      });
      return {
        x: xs, y: ys, text: texts, name: b,
        mode: "markers", type: "scatter",
        marker: { color: backendColor(b), size: 7, opacity: 0.7 },
  ...
  function memoryBars(rows) {
    ...
    const traces = present.map(function (b) {
      return {
        x: fams, type: "bar", name: b,
        offsetgroup: b,
        y: fams.map(function (f) {
          const vals = byFamily[f]
            .filter(function (r) { return r.backend === b; })
            .map(function (r) { return r.sc_peak_bytes; });
          return med(vals);
        }),
        marker: { color: backendColor(b) },
      };
    ...
    const layout = Object.assign({}, LAYOUT_BASE, {
      title: "5. Median peak Python allocations per family, grouped by backend (log-y)",
      barmode: "group",
      yaxis: { type: "log", title: "peak bytes" },
      xaxis: { title: "family" },
      showlegend: true,
      legend: { orientation: "h", y: -0.2 },
    });
      ...
      if (!cases.length) {
        ...
        oh.innerHTML = cases.map(function (c) {
          return "<li>" + escapeHtml(c.operation_name) +
                 ...
                 " &mdash; " + fmtNsShort(c.sc_median_ns) +
                 " &middot; " + fmtSpeedup(c.speedup) +
...
<div class="plot-grid">
  ...
  <div id="plot-boxplot" class="plot"></div>
  <div id="plot-scaling" class="plot wide"></div>
