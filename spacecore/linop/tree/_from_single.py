...
class StackedLinOp(TreeLinOp[Domain, TreeSpace]):
    ...
    ``X`` to ``Yi``. Forward application returns a value with ``cod.treedef``;
    ...
    def _check_layout(self) -> None:
        ...
        if not isinstance(self.cod, TreeSpace):
            raise TypeError("StackedLinOp expects cod to be TreeSpace.")

        if len(self.parts) != self.cod.arity:
            raise ValueError("Number of ops must match codomain tree arity.")

        for i, A in enumerate(self.parts):
            if A.dom == self.dom and A.cod == self.cod.leaf_spaces[i]:
                continue
            else:
                raise TypeError(f"Component op {i} must map dom -> cod.leaf_spaces[{i}].")
    ...
    def from_operators(cls, parts: Tuple[LinOp, ...]) -> StackedLinOp:
        ...
        cod = TreeSpace(tuple(range(len(parts))), tuple(op.cod for op in parts))
        dom = parts[0].dom
