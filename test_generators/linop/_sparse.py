...
def make_sparse_linop_data(
    ...
    *,
    ...
    """Generate SparseLinOp reference data using NumPy first, then ``ctx``."""
    ...
    density = min(0.7, max(0.2, 12.0 / max(domain_size * codomain_size, 1)))
    A_np = sps.random(
        codomain_size,
        domain_size,
        density=density,
...
def bare_sparse_linop(
    ops: BackendOps,
    data: SparseLinOpTestData,
    kind: Literal["apply", "rapply", "vapply", "rvapply"],
    *,
    time: bool = False,
    jit: bool = False,
) -> DenseArray:
    """Compute a sparse LinOp reference directly with backend array operations."""
    ...
    if weighted and (data.domain_weights is None or data.codomain_weights is None):
        raise ValueError("Weighted sparse reference requires both domain and codomain weights.")
    ...
    if kind == "apply":
        ...
        def kernel(A2: SparseArray, x2: DenseArray) -> DenseArray:
            return A2 @ x2
        ...
        if weighted:
            ...
            def kernel(A2H: SparseArray, y2: DenseArray) -> DenseArray:
                return A2H @ y2
        ...
        def kernel(A2: SparseArray, xs2: DenseArray) -> DenseArray:
            return (A2 @ xs2.T).T
        ...
        if weighted:
            ...
            def kernel(
                ...
                return (A2H @ (wy2 * ys2).T).T / wx2

            args = (AH, ys_flat, wy, wx)
        else:

            def kernel(A2H: SparseArray, ys2: DenseArray) -> DenseArray:
                return (A2H @ ys2.T).T

            args = (AH, ys_flat)
        out_shape = (data.ys.shape[0],) + domain_shape
    else:
        raise ValueError(f"Unknown sparse reference kind: {kind!r}.")
...
def check_sparse_linop(
    ...
    *,
    ...
    """Run SparseLinOp and backend-only references, storing outputs and timings."""
    op = SparseLinOp(data.operator, data.domain, data.codomain, data.ctx)
    key = kind if not jit else f"{kind}:jit"
    bare = bare_sparse_linop(data.ctx.ops, data, kind, time=time, jit=jit)
...
def _sparse_conj(A: SparseArray) -> SparseArray:
    if hasattr(A, "conj"):
        return A.conj()
...
def _spacecore_kernel(
    ...
    *,
    ...
    if kind == "apply":
        ...
        if data.xs is None:
            raise ValueError("check_sparse_linop(kind='vapply') requires data.xs.")
        method = op._vapply_core if jit else op.vapply
        arg = data.xs
    elif kind == "rvapply":
        if data.ys is None:
            raise ValueError("check_sparse_linop(kind='rvapply') requires data.ys.")
