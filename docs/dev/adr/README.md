...
| [ADR-021](021_lazy_operator_algebra_and_simplification.md) | `021_lazy_operator_algebra_and_simplification` | Proposed | Tier-2 explicit `LinOp.fuse(*, materialize=False)` simplification; collapses maximal dense subtrees into one `DenseLinOp` (composition→matrix product, scalar/sum/adjoint/block/tree), adjoint-consistent on any geometry, matrix-free never densified unless `materialize=True`; implemented in `spacecore.linop`. |
| [ADR-022](022_caching.md) | `022_caching` | Proposed | Build-time materialized-form cache for the uniform batched folds: the input-independent stacked block-matrix array is memoized once per accessor on `CachedStackParts` (excluded from identity/pytree, matrix-free never cached, dispatcher stateless); implemented for `BlockDiagonalLinOp`/`StackedLinOp`/`SumToSingleLinOp`. Dispatch-decision caching deferred. |
