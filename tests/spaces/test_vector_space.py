...
def test_vector_space_zeros_add_scale_inner():
    ...
    assert np.allclose(X.add(x, y), [5.0, 7.0, 9.0])
    assert np.allclose(X.scale(2.0, x), [2.0, 4.0, 6.0])
    assert np.allclose(X.inner(x, y), 32.0)
...
def test_vector_space_check_member():
    ...
    X.check_member(ctx.asarray([1.0, 2.0]))
    with pytest.raises(Exception):
        X.check_member(np.asarray([1.0, 2.0, 3.0], dtype=np.float32))
