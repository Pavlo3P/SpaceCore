...
def test_product_space_construction_and_shape():
    ...
    P = sc.ProductSpace((sc.DenseCoordinateSpace((2,), ctx), sc.DenseCoordinateSpace((3,), ctx)), ctx)
    assert P.shape == (5,)
...
def test_product_space_uses_resolved_context_dtype_for_components():
    ...
    P = sc.ProductSpace((sc.DenseCoordinateSpace((2,), c1), sc.DenseCoordinateSpace((3,), c2)), c2)
...
def test_product_space_convert_changes_backend_only_if_supported():
    ...
    sc = importlib.import_module("spacecore")
    dt = jax_real_dtype()
    P = sc.ProductSpace((sc.DenseCoordinateSpace((2,), sc.Context(sc.NumpyOps(), dtype=dt)), sc.DenseCoordinateSpace((3,), sc.Context(sc.NumpyOps(), dtype=dt))))
