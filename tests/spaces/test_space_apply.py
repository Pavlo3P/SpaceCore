...
from spacecore.space import ElementwiseJordanSpace, HermitianSpace, TreeSpace
...
def test_tree_spectral_apply_numpy_leafwise():
    sp1 = ElementwiseJordanSpace((2,), ctx=_np_ctx())
    sp2 = ElementwiseJordanSpace((3,), ctx=_np_ctx())
    psp = TreeSpace.from_leaf_spaces((sp1, sp2), ctx=_np_ctx())

    x = (
        np.asarray([1.0, 2.0], dtype=psp.leaf_spaces[0].dtype),
        np.asarray([3.0, 4.0, 5.0], dtype=psp.leaf_spaces[1].dtype),
    )

    y = psp.spectral_apply(x, np.square)

    np.testing.assert_allclose(
        y[0], np.asarray([1.0, 4.0], dtype=psp.leaf_spaces[0].dtype)
    )
    np.testing.assert_allclose(
        y[1], np.asarray([9.0, 16.0, 25.0], dtype=psp.leaf_spaces[1].dtype)
    )
...
def test_tree_spectral_apply_jax_matches_eager_and_compiles():
    jax = pytest.importorskip("jax")
    jnp = pytest.importorskip("jax.numpy")

    sp1 = ElementwiseJordanSpace((2,), ctx=_jax_ctx())
    sp2 = ElementwiseJordanSpace((3,), ctx=_jax_ctx())
    psp = TreeSpace.from_leaf_spaces((sp1, sp2), ctx=_jax_ctx())

    x = (
        jnp.asarray([1.0, 2.0], dtype=psp.leaf_spaces[0].dtype),
        jnp.asarray([3.0, 4.0, 5.0], dtype=psp.leaf_spaces[1].dtype),
    )
    f = jnp.square
