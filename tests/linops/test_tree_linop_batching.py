...
def test_space_add_batch_for_vector_and_nested_tree_space():
    ctx = sc.Context(sc.NumpyOps(), dtype=np.float64)
    vector = sc.DenseCoordinateSpace((2,), ctx)
    tree = sc.TreeSpace.from_template(
        (0, (0, 0)),
        (vector, sc.DenseCoordinateSpace((1,), ctx), vector),
        ctx=ctx,
    )
...
def test_tree_linops_use_space_add_batch_for_accumulation():
    ctx = sc.Context(sc.NumpyOps(), dtype=np.float64)
...
def test_stacked_adjoint_identity_and_batched_rvapply_with_weighted_space():
    ...
    rows = tuple(op.rapply(_slice_tree_batch(ys, i)) for i in range(2))
    np.testing.assert_allclose(to_numpy(op.rvapply(ys)), _stack_tree_rows(rows))
...
def test_sum_to_single_adjoint_identity_and_batched_vapply_with_weighted_space():
    ...
    rows = tuple(op.apply(_slice_tree_batch(xs, i)) for i in range(2))
    np.testing.assert_allclose(to_numpy(op.vapply(xs)), _stack_tree_rows(rows))


def test_tree_linop_batch_checks_reject_wrong_tuple_layout():
