...
def test_block_diagonal_accepts_and_returns_structured_tree_elements(kind):
    ctx = _ctx(kind)
...
@pytest.mark.parametrize("kind", ["numpy", "jax"])
def test_tree_linop_batch_paths_use_tree_of_batched_leaves(kind):
    ctx = _ctx(kind)

    block_parts = _block_parts(ctx)
    block_dom = _structured_tree(
        (block_parts[0].domain, block_parts[1].domain),
        State(ctx.asarray([0.0, 0.0]), ctx.asarray([0.0, 0.0, 0.0])),
        ctx,
    )
    block_cod = _structured_tree(
