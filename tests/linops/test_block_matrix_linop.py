...
def test_apply_rapply_inference_and_structural_adjoint():
    ctx = _ctx()
    op = sc.BlockMatrixLinOp(_blocks(ctx))
    x = (ctx.asarray([1.0, 2.0]), ctx.asarray([3.0]))
    y = (ctx.asarray([2.0]), ctx.asarray([1.0, -2.0]))
...
def test_non_euclidean_metric_adjoint_identity():
    ctx = _ctx()
    op = sc.BlockMatrixLinOp(_blocks(ctx, weighted=True))
    x = (ctx.asarray([1.0, -2.0]), ctx.asarray([3.0]))
    y = (ctx.asarray([0.5]), ctx.asarray([2.0, -1.0]))
...
def test_batched_apply_rapply_and_algebra():
    ctx = _ctx()
    op = sc.BlockMatrixLinOp(_blocks(ctx))
    xs = (ctx.asarray([[1.0, 2.0], [3.0, 4.0]]), ctx.asarray([[3.0], [-1.0]]))
    ys = (ctx.asarray([[2.0], [-1.0]]), ctx.asarray([[1.0, -2.0], [3.0, 4.0]]))
    ...
    normal = op.H @ op
    _assert_tuple_allclose(normal.apply((xs[0][0], xs[1][0])), op.rapply(op.apply((xs[0][0], xs[1][0]))))


def test_rejects_invalid_block_layouts_and_contexts():
    ctx = _ctx()
    rows = _blocks(ctx)
    ...
    wrong_cod = sc.DenseCoordinateSpace((2,), ctx)
    incompatible_row = sc.IdentityLinOp(wrong_cod)
    with pytest.raises(ValueError, match="row 0"):
        sc.BlockMatrixLinOp(((rows[0][0], incompatible_row),))

    wrong_dom = sc.DenseCoordinateSpace((2,), ctx)
    incompatible_column = sc.DenseLinOp(
        ctx.asarray([[1.0, 0.0], [0.0, 1.0]]), wrong_dom, rows[1][0].codomain, ctx
    )
