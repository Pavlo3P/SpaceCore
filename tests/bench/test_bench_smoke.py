...
def _mock_result(name: str, family: str, speedup: float, *, err: float = 0.0) -> ProbeResult:
...
def test_compare_to_baseline_flags_regression():
    ...
    assert verdict.statuses["op@256/numpy/cpu"] == Status.REGRESSION
...
def test_run_probes_smoke_for_one_probe(monkeypatch):
    ...
    assert r.unchecked_median_ns is not None
