...
def test_cg_uses_weighted_inner_products_and_residual_norms():
    ...
    np.testing.assert_allclose(
        to_numpy(result.residual_norm), to_numpy(space.norm(residual)), atol=1e-12
    )
...
def test_power_iteration_uses_metric_rayleigh_quotient_and_norm():
    ...
    rayleigh = space.inner(result.eigenvector, Ax) / space.inner(
    ...
    np.testing.assert_allclose(
        to_numpy(result.eigenvalue), to_numpy(rayleigh), rtol=1e-10, atol=1e-10
    )
