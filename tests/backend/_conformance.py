"""Shared harness for backend conformance tests.

The conformance suite compares each backend's behavior against a NumPy
reference. Tests parametrize over the ``backend_ops`` fixture (defined in
...
Backends that are not installed are skipped at fixture-collection time; a
test that requires a specific backend can also skip itself via
``pytest.skip(...)`` when ``ops.family`` does not match.
"""
